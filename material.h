...
vec3 randomInUnitSphere(float rand1, float rand2, float rand3) {
    vec3 p;
    // do {        //3 randoms
    //     // p=2.0*vec3(((double) rand()/RAND_MAX),((double) rand()/RAND_MAX),((double) rand()/RAND_MAX)) - vec3(1,1,1);
    //     p=2.0*vec3(rand1, rand2, rand3) - vec3(1,1,1);
...
class material  {
    public:
        virtual bool scatter(const ray& r_in, const hitRecord& rec, vec3& attenuation, ray& scattered,float rand1, float rand2, float rand3) const=0;
};

class matte : public material {
    public:
        matte(const vec3& a) : albedo(a) {}
        virtual bool scatter(const ray& r_in, const hitRecord& rec, vec3& attenuation, ray& scattered,float rand1, float rand2, float rand3) const  {
